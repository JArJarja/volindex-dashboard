...
async function loadValuation(s:string):Promise<any>{try{const r=await fetch(`${API}/api/index/${encodeURIComponent(s)}/valuation`);return r.ok?r.json():null}catch{return null}}
...
function analyze48h(prices:PricePoint[],cur:number){
  ...
  if(slopePct>0.05){score+=1;points.push({label:"+",positive:true,text:`Pente haussiere (+${slopePct.toFixed(3)}%/h)`});}
  else if(slopePct<-0.05){score-=1;points.push({label:"−",positive:false,text:`Pente baissiere (${slopePct.toFixed(3)}%/h)`});}
  ...
  if(vol48>2)points.push({label:"~",positive:false,text:`Volatilité horaire elevee ${vol48.toFixed(2)}%`});
  const abs=Math.abs(score);
  const direction:"haussier"|"baissier"|"incertain"=abs<1?"incertain":score>0?"haussier":"baissier";
  const conviction:string=abs>=4?"forte":abs>=2?"moderee":"faible";
  const sum:any={haussier:{forte:"Dynamique clairement haussiere sur 48h.",moderee:"Biais haussier modéré sur 48h.",faible:"Légère inclination haussiere."},baissier:{forte:"Pression vendeuse dominante sur 48h.",moderee:"Biais baissier modéré sur 48h.",faible:"Légère pression baissiere."},incertain:{forte:"Aucune direction dominante.",moderee:"Signaux contradictoires.",faible:"Données insuffisantes."}};
...
function PredictionPanel({data}:{data:Detail}){
  ...
  const conv=abs>=4?"forte":abs>=2?"moderee":"faible";
          ...
          <div key={i} style={{background:"#0f1117",border:`1px solid ${isNext?"#00c49a":"#1e2130"}`,borderRadius:8,padding:"12px 14px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:6,marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                {e.is_future&&<span style={{padding:"2px 6px",borderRadius:3,fontFamily:"monospace",fontSize:9,background:"rgba(0,196,154,.12)",color:"#00c49a"}}>ESTIMATION</span>}
                {e.nb_analysts>0&&<span style={{fontFamily:"monospace",fontSize:9,color:"#7a8098"}}>{e.nb_analysts} analystes</span>}
                {isPast&&<span style={{padding:"2px 6px",borderRadius:3,fontFamily:"monospace",fontSize:9,background:"rgba(122,128,152,.1)",color:"#7a8098"}}>PUBLIÉ</span>}
              </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <div style={{background:"#1e2130",borderRadius:6,padding:"10px 12px",borderLeft:`3px solid ${beatEps===true?"#00c49a":beatEps===false?"#e05060":"#3a3f55"}`}}>
                <div style={{fontFamily:"monospace",fontSize:9,color:"#7a8098",marginBottom:6}}>BPA (Benefice par Action)</div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                  <div>
                    <div style={{fontFamily:"monospace",fontSize:9,color:"#7a8098"}}>Estime</div>
                    <div style={{fontFamily:"monospace",fontSize:13,color:"#c8cfe0"}}>{epsEst?epsEst.toFixed(2):"--"}</div>
                    {e.eps_low>0&&e.eps_high>0&&<div style={{fontFamily:"monospace",fontSize:9,color:"#7a8098"}}>Fourchette: {e.eps_low.toFixed(2)} - {e.eps_high.toFixed(2)}</div>}
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontFamily:"monospace",fontSize:9,color:"#7a8098"}}>Reel</div>
...
function ValuationPanel({valuation,symbol}:{valuation:any;symbol:string}){
  if(!valuation) return(
    <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:20}}>
      <p style={{fontFamily:"monospace",fontSize:11,color:"#7a8098"}}>Chargement de la valorisation...</p>
    </div>
  );
  if(!valuation.available) return(
    <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:20}}>
      <p style={{fontFamily:"monospace",fontSize:11,color:"#7a8098"}}>{valuation.reason||"Valorisation non disponible"}</p>
    </div>
  );

  const verdictColor=valuation.ecart_consensus_pct>10?"#00c49a":valuation.ecart_consensus_pct<-10?"#e05060":"#d49a00";
  const qualiteColor=valuation.score_qualite>=7?"#00c49a":valuation.score_qualite>=4?"#d49a00":"#e05060";

  function EcartBadge({pct}:{pct:number}){
    const c=pct>10?"#00c49a":pct<-10?"#e05060":"#d49a00";
    return <span style={{fontFamily:"monospace",fontSize:11,fontWeight:700,color:c}}>{pct>0?"+":""}{pct.toFixed(1)}%</span>;
  }

  const methodes=valuation.methodes||{};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {/* Verdict principal */}
      <div style={{background:`${verdictColor}0a`,border:`1px solid ${verdictColor}40`,borderRadius:8,padding:"16px 18px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,marginBottom:14}}>
          <div>
            <p style={{fontFamily:"monospace",fontSize:10,color:"#7a8098",marginBottom:4}}>VALEUR INTRINSEQUE CONSENSUS</p>
            <div style={{display:"flex",alignItems:"baseline",gap:12}}>
              <span style={{fontFamily:"monospace",fontSize:26,fontWeight:700,color:verdictColor}}>
                {valuation.valeur_consensus?valuation.valeur_consensus.toFixed(2):"--"}
              </span>
              <span style={{fontFamily:"monospace",fontSize:12,color:"#7a8098"}}>
                vs prix marche {valuation.prix_marche.toFixed(2)}
              </span>
            </div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontFamily:"monospace",fontSize:13,fontWeight:700,color:verdictColor,marginBottom:4}}>
              {valuation.verdict}
            </div>
            <EcartBadge pct={valuation.ecart_consensus_pct}/>
            <span style={{fontFamily:"monospace",fontSize:10,color:"#7a8098",marginLeft:6}}>vs prix actuel</span>
          </div>
        </div>

        {/* Barre de position */}
        {valuation.valeur_consensus>0&&(()=>{
          const lo=Math.min(valuation.prix_marche,valuation.valeur_consensus)*0.85;
          const hi=Math.max(valuation.prix_marche,valuation.valeur_consensus)*1.15;
          const range=hi-lo;
          const pPct=((valuation.prix_marche-lo)/range)*100;
          const vPct=((valuation.valeur_consensus-lo)/range)*100;
          return(
            <div>
              <div style={{position:"relative",height:10,background:"#1e2130",borderRadius:5,marginBottom:6}}>
                <div style={{position:"absolute",left:`${Math.min(pPct,vPct)}%`,width:`${Math.abs(vPct-pPct)}%`,height:"100%",background:`${verdictColor}30`,borderRadius:5}}/>
                <div style={{position:"absolute",left:`${Math.min(98,Math.max(0,vPct))}%`,top:-3,width:4,height:16,background:verdictColor,borderRadius:2,transform:"translateX(-50%)"}}/>
                <div style={{position:"absolute",left:`${Math.min(98,Math.max(0,pPct))}%`,top:-4,width:18,height:18,background:"#c8cfe0",borderRadius:"50%",border:"2px solid #0f1117",transform:"translateX(-50%)"}}/>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",fontFamily:"monospace",fontSize:9,color:"#7a8098"}}>
                <span>{lo.toFixed(2)}</span>
                <span style={{color:verdictColor}}>▲ Valeur intrinseque {valuation.valeur_consensus.toFixed(2)}</span>
                <span>{hi.toFixed(2)}</span>
              </div>
              <div style={{textAlign:"center",fontFamily:"monospace",fontSize:9,color:"#7a8098",marginTop:2}}>● Prix marche {valuation.prix_marche.toFixed(2)}</div>
            </div>
          );
        })()}
      </div>
      ...
      <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:"14px 16px"}}>
        <p style={{fontFamily:"monospace",fontSize:10,color:"#7a8098",marginBottom:12,letterSpacing:".08em"}}>METHODES DE VALORISATION</p>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {Object.values(methodes).map(function(m:any,i:number){
            const mc=m.ecart_pct>10?"#00c49a":m.ecart_pct<-10?"#e05060":"#d49a00";
            return(
              <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",background:"#1e2130",borderRadius:6,borderLeft:`3px solid ${mc}`,flexWrap:"wrap",gap:8}}>
                <div>
                  <div style={{fontFamily:"monospace",fontSize:11,fontWeight:600,color:"#c8cfe0",marginBottom:3}}>{m.methode}</div>
                  <div style={{fontFamily:"monospace",fontSize:9,color:mc}}>{m.interpretation}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontFamily:"monospace",fontSize:16,fontWeight:700,color:mc}}>{m.valeur_intrinseque.toFixed(2)}</div>
                  <EcartBadge pct={m.ecart_pct}/>
                </div>
              </div>
            );
          })}
          {Object.keys(methodes).length===0&&(
            <p style={{fontFamily:"monospace",fontSize:11,color:"#7a8098"}}>Donnees insuffisantes pour calculer les methodes de valorisation</p>
          )}
        </div>
      </div>

      {/* Score qualite fondamentale */}
      <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:"14px 16px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          <p style={{fontFamily:"monospace",fontSize:10,color:"#7a8098",letterSpacing:".08em"}}>QUALITE FONDAMENTALE</p>
          <span style={{fontFamily:"monospace",fontSize:12,fontWeight:700,padding:"3px 10px",borderRadius:4,color:qualiteColor,background:`${qualiteColor}18`,border:`1px solid ${qualiteColor}40`}}>
            {valuation.qualite_fondamentale} ({valuation.score_qualite}/10)
          </span>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:5}}>
          {(valuation.facteurs_qualite||[]).map(function(f:string,i:number){
            const pos=!f.includes("faible")&&!f.includes("baisse")&&!f.includes("tendue")&&!f.includes("eleve")&&!f.includes("surevalu");
            return(
              <div key={i} style={{display:"flex",gap:8,fontSize:12,color:"#c8cfe0",lineHeight:1.5}}>
                <span style={{color:pos?"#00c49a":"#e05060",fontWeight:700,flexShrink:0}}>{pos?"✓":"✗"}</span>
                <span>{f}</span>
              </div>
            );
          })}
      ...
      {/* Ratios financiers */}
      <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:"14px 16px"}}>
        <p style={{fontFamily:"monospace",fontSize:10,color:"#7a8098",marginBottom:12,letterSpacing:".08em"}}>RATIOS FINANCIERS CLES</p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
          {[
            {l:"PER (TTM)",v:ratios.per_ttm,suf:"x",good:r=>r<20,bad:r=>r>35},
            {l:"PER (Fwd)",v:ratios.per_fwd,suf:"x",good:r=>r<18,bad:r=>r>30},
            {l:"PEG",v:ratios.peg,suf:"x",good:r=>r<1,bad:r=>r>2},
            {l:"Price/Book",v:ratios.pbv,suf:"x",good:r=>r<2,bad:r=>r>5},
            {l:"EV/EBITDA",v:ratios.ev_ebitda,suf:"x",good:r=>r<12,bad:r=>r>25},
            {l:"ROE",v:ratios.roe,suf:"%",good:r=>r>15,bad:r=>r<5},
            {l:"ROA",v:ratios.roa,suf:"%",good:r=>r>8,bad:r=>r<2},
            {l:"Marge nette",v:ratios.marge_nette,suf:"%",good:r=>r>15,bad:r=>r<5},
            {l:"Croiss. CA",v:ratios.croissance_ca,suf:"%",good:r=>r>10,bad:r=>r<0},
            {l:"Dette/Cap.",v:ratios.dette_capitaux,suf:"%",good:r=>r<50,bad:r=>r>150},
            {l:"Rendement div.",v:ratios.rendement_div,suf:"%",good:r=>r>2,bad:r=>r===0},
            {l:"Beta",v:ratios.beta,suf:"",good:r=>r<1,bad:r=>r>1.5},
          ].map(function(r){
            if(r.v===null||r.v===undefined) return null;
            const color=r.good&&r.good(r.v)?"#00c49a":r.bad&&r.bad(r.v)?"#e05060":"#d49a00";
            return(
              <div key={r.l} style={{background:"#1e2130",borderRadius:6,padding:"8px 10px"}}>
                <div style={{fontFamily:"monospace",fontSize:9,color:"#7a8098",marginBottom:3}}>{r.l}</div>
                <div style={{fontFamily:"monospace",fontSize:13,fontWeight:700,color}}>{r.v.toFixed(2)}{r.suf}</div>
              </div>
            );
          }).filter(Boolean)}
        </div>
        <p style={{fontFamily:"monospace",fontSize:9,color:"#3a3f55",marginTop:12,lineHeight:1.6}}>
          WACC utilise: {ratios.wacc}% | Source: Yahoo Finance | Modeles DCF, PER, DDM, Price-to-Book
        </p>
      </div>

      {/* Disclaimer */}
      <div style={{background:"rgba(122,128,152,.06)",border:"1px solid #1e2130",borderRadius:6,padding:"10px 14px"}}>
...
// -- Actualites et Consensus --
function NewsConsensusPanel({news,consensus,symbol,price}:{news:any[];consensus:any;symbol:string;price:number}){
  const[tab,setTab]=useState<string>("consensus");

  function getSentColor(s:string){
    if(s==="positif") return "#00c49a";
    if(s==="negatif") return "#e05060";
    return "#7a8098";
  }
  function getSentBg(s:string){
    if(s==="positif") return "rgba(0,196,154,.1)";
    if(s==="negatif") return "rgba(224,80,96,.1)";
    return "rgba(122,128,152,.1)";
  }

  const tabs=[["consensus","Consensus analystes"],["news","Actualites"],["social","Sentiment social"]];
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {tab==="consensus"&&(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {!consensus||consensus.type==="none"?(
            <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:16}}>
              <p style={{fontFamily:"monospace",fontSize:11,color:"#7a8098"}}>Consensus non disponible — configurez FMP_API_KEY</p>
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {/* Verdict */}
              <div style={{background:consensus.verdict==="haussier"?"rgba(0,196,154,.06)":consensus.verdict==="baissier"?"rgba(224,80,96,.06)":"rgba(212,154,0,.06)",border:`1px solid ${consensus.verdict==="haussier"?"rgba(0,196,154,.3)":consensus.verdict==="baissier"?"rgba(224,80,96,.3)":"rgba(212,154,0,.3)"}`,borderRadius:8,padding:"14px 16px"}}>
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10,flexWrap:"wrap"}}>
                  <span style={{fontFamily:"monospace",fontSize:15,fontWeight:700,color:consensus.verdict==="haussier"?"#00c49a":consensus.verdict==="baissier"?"#e05060":"#d49a00",textTransform:"uppercase"}}>
                    {consensus.verdict==="haussier"?"CONSENSUS HAUSSIER":consensus.verdict==="baissier"?"CONSENSUS BAISSIER":"CONSENSUS NEUTRE"}
                  </span>
                  <span style={{fontFamily:"monospace",fontSize:10,color:"#7a8098"}}>{consensus.total||0} analystes</span>
                </div>
                {/* Barre Buy/Hold/Sell */}
                {(consensus.strong_buy||consensus.buy)&&(
                  <div>
                    <div style={{display:"flex",height:20,borderRadius:4,overflow:"hidden",gap:1,marginBottom:6}}>
                      {(consensus.strong_buy||0)>0&&<div style={{flex:consensus.strong_buy,background:"#00c49a",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontFamily:"monospace",color:"#000",fontWeight:700}}>SB</div>}
                      {(consensus.buy||0)>0&&<div style={{flex:consensus.buy,background:"rgba(0,196,154,.5)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontFamily:"monospace",color:"#000"}}>B</div>}
                      {(consensus.hold||0)>0&&<div style={{flex:consensus.hold,background:"#3a3f55",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontFamily:"monospace",color:"#c8cfe0"}}>H</div>}
                      {(consensus.sell||0)>0&&<div style={{flex:consensus.sell,background:"rgba(224,80,96,.5)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontFamily:"monospace",color:"#fff"}}>S</div>}
                      {(consensus.strong_sell||0)>0&&<div style={{flex:consensus.strong_sell,background:"#e05060",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontFamily:"monospace",color:"#fff",fontWeight:700}}>SS</div>}
                    </div>
                    <div style={{display:"flex",justifyContent:"space-between",fontFamily:"monospace",fontSize:10}}>
                      <span style={{color:"#00c49a"}}>Achat {consensus.bull_pct||0}%</span>
                      <span style={{color:"#7a8098"}}>Conserver {consensus.hold_pct||0}%</span>
                      <span style={{color:"#e05060"}}>Vente {consensus.bear_pct||0}%</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              {/* Actions recentes analystes */}
              {consensus.recent_actions&&consensus.recent_actions.length>0&&(
                <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:"12px 14px"}}>
                  <p style={{fontFamily:"monospace",fontSize:10,color:"#7a8098",marginBottom:10}}>DERNIERES ACTIONS ANALYSTES</p>
                  <div style={{display:"flex",flexDirection:"column",gap:5}}>
                    {consensus.recent_actions.map(function(r:any,i:number){
                      const bullGrade=r.to_grade==="Buy"||r.to_grade==="Outperform"||r.to_grade==="Overweight";
                      const bearGrade=r.to_grade==="Sell"||r.to_grade==="Underperform"||r.to_grade==="Underweight";
                      return(
                        <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 8px",background:"#1e2130",borderRadius:4}}>
                          <span style={{fontFamily:"monospace",fontSize:9,color:"#7a8098",minWidth:80}}>{r.date}</span>
                          <span style={{fontFamily:"monospace",fontSize:10,color:"#c8cfe0",flex:1}}>{r.analyst||"Analyste"}</span>
                          {r.from_grade&&<span style={{fontFamily:"monospace",fontSize:9,color:"#7a8098"}}>{r.from_grade} &rarr;</span>}
                          <span style={{fontFamily:"monospace",fontSize:10,fontWeight:600,color:bullGrade?"#00c49a":bearGrade?"#e05060":"#d49a00"}}>{r.to_grade||r.action}</span>
                        </div>
                      );
                    })}
                  </div>
      ...
      {tab==="news"&&(
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {news.length===0?(
            <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:16}}>
              <p style={{fontFamily:"monospace",fontSize:11,color:"#7a8098"}}>Aucune actualite disponible</p>
            </div>
          ):news.map(function(n:any,i:number){
            return(
              <a key={i} href={n.url} target="_blank" rel="noopener noreferrer" style={{textDecoration:"none"}}>
                <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:"12px 14px",cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:6}}>
                    <div style={{display:"flex",gap:8}}>
                      <span style={{fontFamily:"monospace",fontSize:9,color:"#7a8098"}}>{n.source}</span>
                      <span style={{fontFamily:"monospace",fontSize:9,color:"#7a8098"}}>{n.datetime>0?new Date(n.datetime*1000).toLocaleDateString("fr-FR",{day:"2-digit",month:"short"}):"" }</span>
                    </div>
                    <span style={{padding:"2px 6px",borderRadius:3,fontFamily:"monospace",fontSize:9,fontWeight:600,color:getSentColor(n.sentiment),background:getSentBg(n.sentiment)}}>
                      {n.sentiment}
                    </span>
                  </div>
                  ...
                  <p style={{fontFamily:"monospace",fontSize:9,color:"#00c49a",marginTop:6}}>Lire l'article &rarr;</p>
                </div>
              </a>
            );
          })}
      ...
      {tab==="social"&&(
        <div style={{background:"#0f1117",border:"1px solid #1e2130",borderRadius:8,padding:"14px 16px"}}>
          <p style={{fontFamily:"monospace",fontSize:10,color:"#7a8098",marginBottom:12}}>SENTIMENT SOCIAL</p>
          {!consensus?(
            <p style={{fontFamily:"monospace",fontSize:11,color:"#7a8098"}}>Donnees non disponibles</p>
            ...
            <div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",fontFamily:"monospace",fontSize:11}}>
                <span style={{color:"#00c49a"}}>Positif {(consensus.sentiment_up_pct||0).toFixed(1)}%</span>
                <span style={{color:"#e05060"}}>Negatif {(consensus.sentiment_down_pct||0).toFixed(1)}%</span>
              </div>
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          <p style={{fontFamily:"monospace",fontSize:10,color:"#7a8098",marginTop:10,lineHeight:1.6}}>
            Analyse informative uniquement. Ne constitue pas un conseil en investissement.
          </p>
        </div>
      )}
    </div>
...
// -- Page principale --
type SectionKey="principal"|"actualites"|"technique"|"valorisation"|"resultats"|"dividendes";
...
export default function DetailPage(){
  ...
  async function load(sym:string,res:string,w:number){
    ...
    try{
      ...
      const[h,e,dv,nw,cs,sent,val]=await Promise.all([loadHistory48(sym),loadEarnings(sym),loadDividends(sym),loadNews(sym),loadConsensus(sym),loadSentiment(sym),loadValuation(sym)]);
      setPrices48(h);setEarnings(e);setDividends(dv);setNews(nw);setConsensus(cs);setSentiment(sent);setValuation(val);
  ...
  const sections:{key:SectionKey;label:string}[]=[
    ...
    ...(!isCrypto?[
      {key:"valorisation" as SectionKey,label:"⚖️ Valorisation"},
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        {data&&(
          <>
            ...
            {section==="valorisation"&&<ValuationPanel valuation={valuation} symbol={symbol}/>}
